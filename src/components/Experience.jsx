  ...
  const experiences = [
    ...
    },
    {
      ...
      highlights: [
        ...
        "Instructed Java programming, computational complexity, OOP paradigms, and coding best practices at Saveetha University and MGR University.",
      ...
      <div className="container">
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '2.5rem', position: 'relative' }}>
          ...
          {experiences.map((exp, idx) => {
            ...
            const isCollegeMatch = selectedCollege && exp.colleges && exp.colleges.some(c => c.toLowerCase().includes(selectedCollege.toLowerCase()));
            const isTopicMatch = selectedTopic && exp.topic && exp.topic.toLowerCase().includes(selectedTopic.toLowerCase());

            const isHighlighted = isVendorMatch || isCollegeMatch || isTopicMatch;
                ...
                {exp.colleges.length > 0 && (
                  <div style={{ marginTop: '1.25rem', borderTop: '1px solid rgba(186, 230, 253, 0.1)', paddingTop: '1rem' }}>
                    ...
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem' }}>
                      {exp.colleges.map(c => {
                        const isColHighlight = selectedCollege && c.toLowerCase().includes(selectedCollege.toLowerCase());
                        const belongsToSelectedVendor = selectedVendor && selectedVendor.colleges.some(vc => c.toLowerCase().includes(vc.toLowerCase()));
